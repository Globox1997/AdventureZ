...
public class KeybindInit {
  ...
  public static void init() {
    ...
    dragonFlyDownKeyBind = new KeyBinding("key.adventurez.dragonflydown", InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_LEFT_ALT,
        "category.adventurez.keybind");
    dragonFireBreathBind = new KeyBinding("key.adventurez.dragonfirebreath", InputUtil.Type.KEYSYM,
        GLFW.GLFW_KEY_GRAVE_ACCENT, "category.adventurez.keybind");
    // Registering
  ...
  public static void dragonFireBreath(ClientPlayerEntity player) {
    if (player.getVehicle() != null && player.getVehicle() instanceof DragonEntity && player.getVehicle().isAlive()) {
      ...
      if (dragonEntity.getDataTracker().get(DragonEntity.DRAGON_SIZE) >= 3) {
        ...
        buf.writeInt(player.getVehicle().getEntityId());
