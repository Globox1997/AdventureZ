...
public class DragonFindOwnerGoal extends Goal {
    ...
    public boolean shouldContinue() {
        if (this.dragonEntity.squaredDistanceTo(this.owner) > 800.0D) {
            ...
            return this.dragonEntity.squaredDistanceTo(this.owner) > (double) (this.maxDistance * this.maxDistance)
                    && !dragonEntity.isOnGround() && dragonEntity.isFlying;
        }
    }
    ...
    public void tick() {
        ...
        if (--this.updateCountdownTicks <= 0) {
            ...
            if (!this.dragonEntity.isLeashed() && !this.dragonEntity.hasVehicle()) {
                if (this.dragonEntity.squaredDistanceTo(this.owner) <= 800.0D) {
                    Vec3d vec3d = new Vec3d(this.owner.getX() - this.dragonEntity.getX(),
                            this.owner.getY() - this.dragonEntity.getY(), this.owner.getZ() - this.dragonEntity.getZ());
