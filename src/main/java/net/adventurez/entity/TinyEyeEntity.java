...
public class TinyEyeEntity extends ExplosiveProjectileEntity {
    ...
    public void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (!this.world.isClient && entity != null && hittedEntity != entity && !(hittedEntity instanceof TinyEyeEntity)
            ...
            if (this.world.isClient) {
                for (int i = 0; i < 20; i++) {
                    double x = MathHelper.nextDouble(random, this.getBoundingBox().minX, entity.getBoundingBox().maxX);
                    double z = MathHelper.nextDouble(random, this.getBoundingBox().minZ, this.getBoundingBox().maxZ);
                    this.world.addParticle(ParticleTypes.PORTAL, x, this.getRandomBodyY(), z, 0.0D, 0.0D, 0.0D);
                }
            }
            this.remove();
        }
        ...
        // LivingEntity livingEntity = entity2 instanceof LivingEntity ? (LivingEntity)
        // entity2 : null;
        // boolean bl = entity.damage(DamageSource.mobProjectile(this,
        // livingEntity).setProjectile(), 4.0F);
        // if (bl) {
        // this.dealDamage(livingEntity, entity);
        // if (entity instanceof LivingEntity) {
        // ((LivingEntity) entity).addStatusEffect(new
    ...
    public void onBlockHit(BlockHitResult blockHitResult) {
        ...
        System.out.print("GotRemovedBlock:");
        this.playSound(SoundEvents.ENTITY_ENDER_EYE_DEATH, 1.0F, 1.0F);
        this.remove();
    }
    ...
    // if (!this.world.isClient) {
    // // System.out.print("GotRemovedDamage");
    // // this.playSound(SoundEvents.ENTITY_SHULKER_BULLET_HURT, 1.0F, 1.0F);
    // // ((ServerWorld) this.world).spawnParticles(ParticleTypes.CRIT, this.getX(),
    // // this.getY(), this.getZ(), 15,
    // // 0.2D, 0.2D, 0.2D, 0.0D);
    // // this.remove();
    // }
