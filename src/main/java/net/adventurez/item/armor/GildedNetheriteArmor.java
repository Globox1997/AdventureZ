...
public class GildedNetheriteArmor extends ArmorItem {
    ...
    public GildedNetheriteArmor(ArmorMaterial material, Type type) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (stack.getItem() == ItemInit.GILDED_NETHERITE_CHESTPLATE) {
            ...
            if (tag != null && tag.contains("armor_time")) {
                if (tag.getBoolean("activating_armor") && tag.getInt("armor_time") + (ConfigInit.CONFIG.gilded_netherite_armor_effect_duration * 2) < (int) world.getTime()) {
                    tag.putBoolean("activating_armor", false);
                }
                if (tag.getBoolean("activating_armor") && tag.getInt("armor_time") + ConfigInit.CONFIG.gilded_netherite_armor_effect_duration < (int) world.getTime()) {
    ...
    public static void activateStoneGolemArmor(PlayerEntity player, ItemStack stack) {
        ...
        StatusEffectInstance statusEffectInstance = new StatusEffectInstance(StatusEffect.byRawId(12), ConfigInit.CONFIG.gilded_netherite_armor_effect_duration, 0, false, false);
        NbtCompound tag = stack.getNbt();
